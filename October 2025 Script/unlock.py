...
# Gets the current Beijing time from NTP
def get_initial_beijing_time():
    ...
    for server in ntp_servers:
        try:
            ...
            print(col_g + "[Beijing time]: " + Fore.RESET + f"{beijing_time.strftime('%Y-%m-%d %H:%M:%S.%f')}")
            return beijing_time
...
# Waits until the target time considering ping
def wait_until_target_time(start_beijing_time, start_timestamp):
    ...
    print(col_g + "[Offset set]: " + Fore.RESET + f"{feed_time_shift:.2f} ms.")
    ...
    while True:
        ...
        if time_diff.total_seconds() > 1:
            ...
            print(f"Time reached: {current_time.strftime('%Y-%m-%d %H:%M:%S.%f')}. Sending requests...")
            break
        else:
            time.sleep(0.0001)

# Checks if unlocking the account is possible via API
def check_unlock_status(session, cookie_value, device_id):
    try:
        ...
        if response is None:
            print("[Error] Unable to get unlock status.")
            return False
        ...
        if response_data.get("code") == 100004:
            print("[Error] Cookie expired, need to update.")
        ...
        if is_pass == 4:
            if button_state == 1:
                print(col_g + "[Account status]: " + Fore.RESET + "Request can be sent..")
                return True

            elif button_state == 2:
                print(col_g + "[Account status]: " + Fore.RESET + f"Locked for requests until {deadline_format} (Month/Day).")
                ...
                print(col_g + "[Account status]: " + Fore.RESET + "Account created less than 30 days ago..")
            ...
            print(col_g + "[Account status]: " + Fore.RESET + f"Request already approved, unlock possible until {deadline_format}.")
            input("Press Enter to exit...")
            exit()
        else:
...
def main():
    ...
    if check_unlock_status(session, cookie_value, device_id):
        ...
        if start_beijing_time is None:
            print("Failed to establish initial time. Press Enter to exit...")
            input()
            exit()
        ...
        try:
            while True:
                ...
                print(col_g + "[Response]: " + Fore.RESET + f"Response received at {response_time.strftime('%Y-%m-%d %H:%M:%S.%f')} (UTC+8)")
                ...
                try:
                    ...
                    if code == 0:
                        ...
                        if apply_result == 1:
                            ...
                            print(col_g + "[Status]: " + Fore.RESET + f"Request not sent, limit reached. Try again on {deadline_format} (Month/Day).")
                            input("Press Enter to exit...")
                            exit()
                        elif apply_result == 4:
                            deadline_format = data.get("deadline_format", "Not specified")
                            print(col_g + "[Status]: " + Fore.RESET + f"Request not sent, blocked until {deadline_format} (Month/Day).")
                            input("Press Enter to exit...")
                            exit()
                    elif code == 100001:
                        print(col_g + "[Status]: " + Fore.RESET + "Request denied, error in request..")
                        print(col_g + "[FULL RESPONSE]: " + Fore.RESET + f"{json_response}")
                    elif code == 100003:
                        print(col_g + "[Status]: " + Fore.RESET + "Request may be approved, checking status...")
                        print(col_g + "[Full response]: " + Fore.RESET + f"{json_response}")
                        check_unlock_status(session, cookie_value, device_id)
                    elif code is not None:
                        print(col_g + "[Status]: " + Fore.RESET + f"Unknown request state: {code}")
                        print(col_g + "[Full response]: " + Fore.RESET + f"{json_response}")
                    else:
                        print(col_g + "[Error]: " + Fore.RESET + "Response missing required code.")
                        print(col_g + "[Full response]: " + Fore.RESET + f"{json_response}")

                except json.JSONDecodeError:
                    print(col_g + "[Error]: " + Fore.RESET + "Couldn't decode response JSON..")
                    print(col_g + "[Server response]: " + Fore.RESET + f"{response_data}")
                except Exception as e:
                    print(col_g + "[Response handling error]: " + Fore.RESET + f"{e}")
                    continue
